...
const Services = ({ children }) => {
  return (
    <div className='relative grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 max-w-6xl mx-auto py-8 sm:py-12 md:py-16 px-4 sm:px-6 lg:px-8'>
      {React.Children.toArray(children).map((child, i) => {
        return (
          <div key={i} className='group relative'>
            {/* Main Card Container */}
            <div className="relative h-[25vh] sm:h-[38vh] md:h-[40vh] bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/20 shadow-2xl overflow-hidden hover:shadow-indigo-500/25 transition-all duration-300 hover:scale-105">
              ...
              {/* Hover Glow Effect */}
              <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500/5 via-purple-500/5 to-red-500/10 rounded-xl sm:rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10"></div>
