...
try {
  ...
  let get=await form_submit("serviceForm",{name:user.name,email:user.email,id:user.id,contact:data.contact,project:data.project,title:data.title});
  console.log(get);
...
useEffect(()=>{console.log(submitError);
},[submitError])
  ...
  return (
    <div className="min-h-screen bg-gray-100 mt-24 flex items-center justify-center p-4 relative overflow-hidden">
      {/* Tech Grid Background */}
       <div className=' h-full w-full absolute sm:top-8 top-4  -left-6 z-60'> 
             <Link
                href={'/services'}
      ...
      {/* Form Container */}
      <div className="relative z-80 w-full max-w-md">
        <div className="bg-white/9 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 p-8">
          ...
          {/* Form */}
          <form className="space-y-6 z-90" onSubmit={handleSubmit(submit_fnx)}>
            {/* Name Field */}
  ...
  <div className="space-y-2">
     ...
     Project Title
             ...
             <textarea {...register("title",{required:"Cant leave it empty",pattern:{value:/^[^<>]*$/,message:"html tags not alowed"},validate:(val)=>{if(val.length>100){return "Characters limit is 100"}else{return true}}})} className="w-full min-h-[40px] text-black p-4 border rounded-lg border-indigo-800">


             </textarea>
                 <p className="text-gray-600 text-lg">{errors?.title?.message}</p>
            </div>
            {/* Course Selection */}
            <div className="space-y-2">
              <label htmlFor="course" className="block text-lg font-medium text-gray-700">
         Write about your project
             ...
             <textarea {...register("project",{required:"Cant leave it empty",pattern:{value:/^[^<>]*$/,message:"html tags not alowed"}})} className="w-full min-h-[200px] text-black p-4 border rounded-lg border-indigo-800">


             </textarea>
                 <p className="text-gray-600 text-lg">{errors?.project?.message}</p>
            </div>

            {/* Submit Button */}
            <button
              type="submit"
              className="w-full cursor-pointer relative overflow-hidden bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 px-6 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-xl group"
            >
